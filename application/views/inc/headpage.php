<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <meta name="description" content="KKMproject - Temukan penyedia jasa terpercaya untuk segala kebutuhan Anda.">
        <meta name="author" content="verdipratama">
        <title><?php echo $title ?></title>
        <!-- Favicons-->
        <link rel="shortcut icon" href="<?php echo base_url('assets/'); ?>img/favicon.ico" type="image/x-icon">
        <link rel="apple-touch-icon" type="image/x-icon" href="<?php echo base_url('assets/'); ?>img/apple-touch-icon-57x57-precomposed.png">
        <link rel="apple-touch-icon" type="image/x-icon" sizes="72x72" href="<?php echo base_url('assets/'); ?>img/apple-touch-icon-72x72-precomposed.png">
        <link rel="apple-touch-icon" type="image/x-icon" sizes="114x114" href="<?php echo base_url('assets/'); ?>img/apple-touch-icon-114x114-precomposed.png">
        <link rel="apple-touch-icon" type="image/x-icon" sizes="144x144" href="<?php echo base_url('assets/'); ?>img/apple-touch-icon-144x144-precomposed.png">
        <!-- GOOGLE WEB FONT -->
        <link href="https://fonts.googleapis.com/css?family=Poppins:300,400,500,600,700" rel="stylesheet">
        <!-- BASE CSS -->
        <link href="<?php echo base_url('assets/'); ?>css/bootstrap.min.css" rel="stylesheet">
        <link href="<?php echo base_url('assets/'); ?>css/style.css" rel="stylesheet">
        <link href="<?php echo base_url('assets/'); ?>css/vendors.css" rel="stylesheet">
        <link href="<?php echo base_url('assets/'); ?>css/custom.css" rel="stylesheet">
        <link href="<?php echo base_url('assets/'); ?>css/page.css" rel="stylesheet">
    </head>
    <body>
        <div id="page">
        <header class="header header_in is_sticky">
            <div class="container">
                <div class="row">
                    <div class="col-lg-3 col-12">
                        <div id="logo">
                            <a href="<?php base_url(); ?>">
                                <img src="<?php echo base_url('assets/'); ?>img/logost.png"  width="165" height="35" class="logo_normal">
                                <img src="<?php echo base_url('assets/'); ?>img/logost.png" width="165" height="35" alt="" class="logo_sticky">
                            </a>
                        </div>
                    </div>
                    <div class="col-lg-9 col-12">
                        <ul id="top_menu">
                            <li><a href="<?php echo base_url(); ?>hubungi" class="btn_add">Hubungi Kami</a></li>
                            <li><a href="#sign-in-dialog" id="sign-in" class="login" title="Sign In">Sign In</a></li>
                            <li><a href="#0" class="wishlist_bt_top" title="Your wishlist">Your wishlist</a></li>
                        </ul>
                        <!-- /top_menu -->
                        <a href="#menu" class="btn_mobile">
                            <div class="hamburger hamburger--spin" id="hamburger">
                                <div class="hamburger-box">
                                    <div class="hamburger-inner"></div>
                                </div>
                            </div>
                        </a>
                        <nav id="menu" class="main-menu">
                            <ul>
                                <li><span><a href="<?php echo base_url(); ?>">Beranda</a></span></li>
                                <li><span><a href="<?php echo base_url(); ?>bengkel">Bengkel Las</a></span></li>
                                <li><span><a href="<?php echo base_url(); ?>">Profil Desa</a></span></li>
                                <li><span><a href="<?php echo base_url(); ?>blog">Potensi Desa</a></span></li>
                                <li><span><a href="<?php echo base_url(); ?>tentang">Tentang Kami</a></span></li>
                            </ul>
                        </nav>
                    </div>
                </div>
            </div>
        </header>
        <!-- /header -->
